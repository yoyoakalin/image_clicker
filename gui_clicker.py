...
from PyQt5.QtGui import QPainter, QColor, QPen, QScreen, QPixmap
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        thumbnail_label = QLabel('截图预览:')
        self.thumbnail_display = QLabel('未截图')
        self.thumbnail_display.setFixedSize(320, 240)
        ...
        if os.path.exists(config_file):
            try:
                ...
                if 'tesseract_path' in config:
                    self.tesseract_path.setText(config['tesseract_path'])
                    
                # 设置目标文字
                if 'target_text' in config:
                    self.target_text.setText(config['target_text'])
                    
                # 设置区域
                if 'region' in config:
                    self.region_input.setText(config['region'])
                    
                # 设置间隔
                if 'interval' in config and isinstance(config['interval'], int):
                    self.interval_spinbox.setValue(config['interval'])
    ...
    def save_config(self):
        ...
        config = {
            ...
            'target_text': self.target_text.text(),
    ...
    def on_region_selected(self, region):
        ...
        try:
            ...
            # 截取区域图像并显示缩略图
            self.capture_region_screenshot(region)
    ...
    def capture_region_screenshot(self, region):
        """截取区域图像并显示缩略图"""
        try:
            x, y, width, height = region
            screen = QApplication.primaryScreen()
            ...
            # 调整大小以适应缩略图显示区域
            thumbnail = screenshot.scaled(320, 240, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            self.thumbnail_display.setPixmap(thumbnail)
            self.log("已生成区域截图")
        except Exception as e:
            self.log(f"截取区域图像失败: {str(e)}")
